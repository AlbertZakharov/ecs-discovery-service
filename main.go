...
func (t *AugmentedTask) ExporterInformation() []*PrometheusTaskInfo {
	...
	for _, i := range t.Containers {
		...
		if *prometheusDynamicPortDetection {
            ...
            regionRegexp := regexp.MustCompile(`\w+-\w+-\d+`)
			accountRegexp := regexp.MustCompile(`:(\d+):`)
			match := accountRegexp.FindStringSubmatch(*t.TaskArn)
			var extractedNumber string
			extractedNumber = match[1]
			for key, portPathPair := range portPairs {
				labels := labels{
					// Disable unnecessary labels - for more information please contacn - AlbertZakharov
					//TaskArn:       *t.TaskArn,
					Region:        regionRegexp.FindString(*t.TaskArn),
					Account:	     extractedNumber,
